...
Encoder M1(M1_ENC_A, M1_ENC_B); // Encoder Right
Encoder M2(M2_ENC_A, M2_ENC_B); // Encoder Left
...
//Room switch
#define roomSwitchInput1 15
#define roomSwitchInput2 14
...
//int frontMidSensorRAW,frontLeftSensorRAW,frontRightSensorRAW;
int rightSensorMid,leftSensorMid,midSensor,rightSensorOut,leftSensorOut;
//int frontMidSensor,frontLeftSensor,frontRightSensor;
...
void moveHalfUp(){
  AR.write(150, 30, false);
  AL.write(30, 30,false);
}
...
void moveHalfDown(){
  AL.write(75, 50, false);
  AR.write(115, 50, false);
...
void readSensors(){
  ...
  */
  ...
  rightSensorOut = map(rightSensorOutRAW, 830, 100, 1000, 0);
  rightSensorMid = map(rightSensorMidRAW, 970 ,120, 1000, 0);
  leftSensorMid = map(leftSensorMidRAW, 970, 140,  1000, 0);
  leftSensorOut = map(leftSensorOutRAW, 850, 105,1000, 0);
  midSensor = map(midSensorRAW, 980 ,190, 1000, 0);
...
//Serial.begin(115200);
  ...
  for(int i = 4; i < 8; i ++)
    ...
    pinMode(roomSwitchInput1, INPUT_PULLUP);
    pinMode(roomSwitchInput2, INPUT_PULLUP);
    attachPinChangeInterrupt(roomSwitchInput1, setRoomFlag1, FALLING);
    attachPinChangeInterrupt(roomSwitchInput2, setRoomFlag2, FALLING);
