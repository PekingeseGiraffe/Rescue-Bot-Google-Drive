...
void readSensors(){
  ...
  leftSensorOut=analogRead(A1);
   ...
   rightSensorOut = map(rightSensorOut, 605, 80, 1000, 0);
     rightSensorMid = map(rightSensorMid,560 ,80, 1000, 0);
  leftSensorMid = map(leftSensorMid, 850, 105,  1000, 0);
  leftSensorOut = map(leftSensorOut, 485, 70,1000, 0);
  /*
...
void DaniBoost(int speed3,int speed4){
  /*
  int rightSensorMid1 = map(rightSensorMid, 0, 1000, 0, 50);
  int leftSensorMid1 = map(leftSensorMid, 0, 1000, 0, 50);
...
void loop(){
  readSensors();
 // Serial.println(caspitState());
 // Serial.println(digitalRead(caspitInput));
