...
#define DEBUG4 1 // sensors analog values RAW
...
int rightSensorMidRAW,leftSensorMidRAW,midSensor,rightSensorOutRAW,leftSensorOutRAW;
int rightSensorMid,leftSensorMid,midSensorRAW,rightSensorOut,leftSensorOut;
int rightSub,leftSub;

boolean digitalSensor[5];
int analogSensor[5];
int analogSensorRAW[5];
const char* sensorNum[] = {"rightSensorOut(A2) = " , " rightSensorMid(A0) = " , " leftSensorMid(A3) = " , " leftSensorOut(A1) = " , " midSensor(A4) = "};
...
void MotorSpinsCM(){
  M1Spin = M1.read() /CPRcm;
  M2Spin = M2.read() /CPRcm;
}

void driveForSpins(int M1num, int M1Speed, int M2num, int M2Speed){
 int  M2temp=M2.read() /CPR;
 int  M1temp=M1.read()/CPR;

 updateSensors();

    while (M1temp+M1num>M1Spin){
